...
class DataGeneratorSimCLR(data_utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i, row in enumerate(batch_data.iterrows()):
            ...
            img_T1 = preprocess_for_train(img, self.height, self.width,
                                          color_distort=True, crop=True, flip=True, blur=True)
            img_T2 = preprocess_for_train(img, self.height, self.width,
                                          color_distort=True, crop=True, flip=True, blur=True)
            # print("diff:", tf.reduce_mean(tf.abs(img_T1 - img_T2)))
            # fig = plt.figure(figsize=(2, 1))
            # fig.add_subplot(2, 1, 1)
            # plt.imshow(img_T1)
            # fig.add_subplot(2, 1, 2)
            # plt.imshow(img_T2), plt.show()
            if self.VGG:
                img_T1 = tf.dtypes.cast(img_T1 * 255, tf.int32)
                img_T2 = tf.dtypes.cast(img_T2 * 255, tf.int32)
                # img_T1 = preprocess_input(np.asarray(img_T1))     # 0-mean
